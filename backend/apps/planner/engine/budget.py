...
def rank_loops(loops: list[Loop], request: TripRequest) -> list[Loop]:
    ...
    affordable = [
        loop for loop in loops if cost_loop(loop, request, LEAN).total <= request.budget_usd
    ]
    return sorted(affordable, key=lambda loop: loop.month_score, reverse=True)


def build_trip_options(loops: list[Loop], request: TripRequest) -> list[TripOption]:
    """Ranks candidate loops, returns the best-fitting one's 3 tier variants."""
    ranked = rank_loops(loops, request)
    if not ranked:
        return []
    return cost_all_tiers(ranked[0], request)
